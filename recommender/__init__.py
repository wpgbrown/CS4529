...
class RecommendedReviewer:
    def __init__(self, email: Optional[str] = None, names: Union[str, List[str]] = None, score: float = 0, parent: Optional[ReferenceType] = None):
        """
    ...
    def __str__(self):
        return_string = "Recommending %s" % self.email
        if len(self.names):
...
class Names(Iterable, Sized):
    ...
    def add(self, name: str):
        ...
        grandparent._update_index(name, parent.email) # noqa
...
class Recommendations:
    ...
    def _update_index(self, name: str, email: str) -> 'Recommendations':
        """
    ...
    def _merge_reviewer_entries(self, base: RecommendedReviewer, other_entry: RecommendedReviewer, remove_other_entry: bool = True):
        ...
        if remove_other_entry:
            ...
            for name in other_entry.names:
                if other_entry is self._recommendations_by_name[name]:
                    del self._recommendations_by_name[name]

...
class MLPClassifierImplementation(RecommenderImplementation, MLPClassifierImplementationBase):
    ...
    def recommend_using_change_info(self, change_info: dict) -> Recommendations:
        ...
        try:
            predicted_approvers = [approved_X.index.values[i] for i, y in enumerate(self.approved_model.predict(approved_X)) if not y]
            predicted_voters = [voted_X.index.values[i] for i, y in enumerate(self.voted_model.predict(voted_X)) if not y]

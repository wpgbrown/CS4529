...
test_data = {"has changes from last 30 days": {}, "has changes from last 3 months": {}, "has changes from last year": {}, "has changes from all time": {}}
try:
    repos = json.load(open(common.path_relative_to_root("raw_data/mediawiki_repos.json"), "r")).keys()
    for number_processed, repo in enumerate(repos):
        ...
        for name, time_unit in {"has changes from last 30 days": relativedelta(days=30), "has changes from last 3 months": relativedelta(months=3), "has changes from last year": relativedelta(years=1), "has changes from all time": None}.items():
            ...
            if (len(response['merged']) < 10 or len(response['open']) < 10) and time_unit is not None:

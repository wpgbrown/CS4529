...
try:
    ...
    for number_processed, (repo, associated_groups) in enumerate(repos_and_associated_members['groups_for_repository'].items()):
        ...
        try:
            ...
            response = generate_votes_for_repository(repo, filter=group_member_names, cutoff_time=int(time.mktime(one_year_ago.timetuple()) * 1_000))
            author_vote_count_for_repo[repo].update({
            ...
            one_year_ago = datetime.datetime.now() - relativedelta(months=3)
            author_vote_count_for_repo[repo].update({
                'last 3 months': generate_votes_for_repository(repo, filter=group_member_names, cutoff_time=int(time.mktime(one_year_ago.timetuple()) * 1_000))
            })
    ...
    json.dump(author_vote_count_for_repo, open(common.path_relative_to_root("raw_data/author_votes_for_repos.json"), "w"))